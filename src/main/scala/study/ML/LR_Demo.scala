...
object LR_Demo {
  ...
  val conf = new SparkConf().setMaster("local[1]").setAppName("LogisticRegression3") //设置环境变变量
  val sc = new SparkContext(conf)

  def main(args: Array[String]): Unit = {
    val data = sc.textFile("D:\\TalkingData\\ML\\jd\\demo_filter_11") //设置数据集
    println(data.count())
    ...
    val splits = parseData.randomSplit(Array(0.7, 0.3), seed = 11L) //把测试集分为两部分一部分用来做训练集一部分做校验集
    ...
    * 容错率为trainErr： 0.4
    ...
    //    else
    //      println("患者的胃癌没有几率转移 。")
    val test = sc.textFile("D:\\TalkingData\\ML\\jd\\test_filter_11").map {
      x =>
        val arr = x.split("\t", -1)
        ...
        if (score == 1.0)
          ...
          "0\t" + x
    }.repartition(1).saveAsTextFile("D:\\TalkingData\\ML\\jd\\testResult_11")
